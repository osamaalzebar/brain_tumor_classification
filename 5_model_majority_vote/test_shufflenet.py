# test_shufflenet_brain_tumor.py
import argparse
import csv
...
def build_shufflenet(num_classes: int = 4, pretrained: bool = True) -> nn.Module:
    """Rebuild ShuffleNet V2 (1.0x) with final FC set to `num_classes`—same as training."""
...
def main():
    parser = argparse.ArgumentParser(description="Test fine-tuned ShuffleNet V2 (1.0x) on 4-class brain tumor dataset")
    ...
    parser.add_argument("--ckpt", type=str, default="checkpoints/shufflenet_v2_brain_tumor_best.pth",
    ...
    parser.add_argument("--save_preds", type=str, default="preds_shufflenet_test.csv",
    ...
    per_class_acc = [cm[i][i] / max(1, sum(cm[i])) for i in range(4)]
    print("Per-class accuracy:", [f"{a:.3f}" for a in per_class_acc])
    print("Balanced accuracy:", sum(per_class_acc) / 4)
