# train_shufflenet_brain_tumor.py
import argparse
import time
...
from dataset_shufflenet import MRIBrainTumorCSV, build_transforms, CLASS_NAMES
...
def build_shufflenet(num_classes: int = 4, pretrained: bool = True) -> nn.Module:
    """ShuffleNet V2 1.0x with final FC replaced for `num_classes`."""
    weights = ShuffleNet_V2_X1_0_Weights.IMAGENET1K_V1 if pretrained else None
    model = shufflenet_v2_x1_0(weights=weights)
    in_features = model.fc.in_features
    model.fc = nn.Linear(in_features, num_classes)  # replaces FC (softmax handled by CrossEntropyLoss)
    return model
...
def train_one_epoch(model, loader, optimizer, device, scaler=None, criterion=None):
    ...
    for images, targets in loader:
        ...
        tot_acc  += accuracy(logits.detach(), targets) * bs
        n += bs
...
def validate(model, loader, device, criterion=None):
    ...
    for images, targets in loader:
        ...
        bs = targets.size(0)
        tot_loss += loss.item() * bs
        tot_acc  += accuracy(logits, targets) * bs
        n += bs
    return tot_loss / n, tot_acc / n


def main():
    parser = argparse.ArgumentParser(description="Fine-tune ShuffleNet V2 (1.0x) on 4-class brain tumor MRI")
    ...
    parser.add_argument("--save_name", type=str, default="shufflenet_v2_brain_tumor_best.pth")
    ...
    # Model
    model = build_shufflenet(num_classes=4, pretrained=not args.no_pretrained).to(device)

    # Give the NEW FC layer a higher LR (10×) than transferred layers (mirrors “last three layers” emphasis)
    new_fc = model.fc
    base_params, new_w, new_b = [], [], []
    for p in model.parameters():
        if p is new_fc.weight:
            new_w.append(p)
        elif new_fc.bias is not None and p is new_fc.bias:
            new_b.append(p)
        else:
    ...
    optimizer = torch.optim.Adam([
        ...
        {"params": new_w, "lr": args.lr * 10.0, "weight_decay": args.weight_decay},
        {"params": new_b, "lr": args.lr * 10.0, "weight_decay": 0.0},
    ])
