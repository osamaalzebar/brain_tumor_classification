# fine_tune_xception_brain_tumor.py
import os
import random
...
# -----------------------
# Config (paths same pattern as before)
# -----------------------
...
OUTPUT_DIR     = "./outputs_xception"
os.makedirs(OUTPUT_DIR, exist_ok=True)
BEST_MODEL_PATH = os.path.join(OUTPUT_DIR, "xception_best.pth")
...
# -----------------------
# Transforms (ImageNet mean/std) + your RandomRotate90
# -----------------------
...
# -----------------------
# Model: Xception (timm) with 4-class head
# -----------------------
def build_xception(num_classes: int = 4) -> nn.Module:
    model = timm.create_model("xception", pretrained=True)
    # timm models expose reset_classifier for safe head replacement
...
def main():
    ...
    model = build_xception(NUM_CLASSES).to(device)
    ...
    # Optional: example softmax probabilities
    model.eval()
    with torch.no_grad():
